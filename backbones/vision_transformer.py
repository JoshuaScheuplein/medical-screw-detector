...
from backbones.utils import trunc_normal_
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False):           # [B, num_patches + 1, embed_dim]
        ...
        """ Original Code (used in combination with method 'get_last_selfattention') """
        # if return_attention:
        #     return attn
        ...
        """ Adapted Code (used in combination with method 'extract_attention') """
        if return_attention:
            return x, attn                                  # [B, num_patches + 1, embed_dim] and [B, num_heads, num_patches + 1, num_patches + 1]
        else:
            return x
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        return x # [B, num_patches, embed_dim] (Note: num_patches is NOT equal to self.num_patches for different input resolutions)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.depth = depth # Additionally added
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        npatch = x.shape[1] - 1             # num_patches for given input image size, e.g., (976, 976)
        N = self.pos_embed.shape[1] - 1     # 'Fixed' num_patches for image size (224, 224)
        # Subtract 1 due to additionally appended CLS token
        ...
        w0 = w // self.patch_embed.patch_size # e.g., 976 // 16 = 61
    ...
    def prepare_tokens(self, x):
        ...
        B, nc, w, h = x.shape # Shape is rather bn, c, w, h ...
        # (B * num_crop_views = bn because of MultiCropWrapper() class)
    ...
    """ Adapted Code (See changes in Block() class) """
    ...
    """ Original Code (See changes in Block() class) """
    # def get_last_selfattention(self, x):
    #     x = self.prepare_tokens(x)
    #     for i, blk in enumerate(self.blocks):
    #         if i < len(self.blocks) - 1:
    ...
    """
    The following function was additionally added
    and NOT contained in the original implementation.
    """
    def extract_attention(self, x):                     # [B, C, W, H]

        x = self.prepare_tokens(x)                      # [B, num_patches + 1, embed_dim]

        attention = []
        for blk in self.blocks:
            x, attn = blk(x, return_attention=True)     # [B, num_heads, num_patches + 1, num_patches + 1]
            attention.append(attn)

        assert len(attention) == self.depth

        return attention                                # num_blocks * [B, num_heads, num_patches + 1, num_patches + 1]
