...
from collections import defaultdict, deque # Deque -> (Doubly Ended Queue)
"""
Deque (Doubly Ended Queue) wird in Python mithilfe des Moduls [collections] implementiert.
Deque ist einer Liste vorzuziehen, wenn wir schnellere Anfüge- und Pop-Operationen
von beiden Enden des Containers benötigen, da Deque eine Zeitkomplexität von O(1) für
Anfüge- und Pop-Operationen bietet, verglichen mit einer Liste mit einer Zeitkomplexität von O(n).
"""
...
"""
Additionally added to debug the following error message: 
...
def num_parameters(model): # Additionally added
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    return np.round((total_params / 1e6), 3)


def bytes_to_mib(bytes): # Additionally added
    return np.round(bytes / (2**20), 3)


# '''
# The following function is used at the start of each training to ensure
# that all GPUs have a CUDA context initialized before calling 'get_gpu_info'.
# '''
# def initialize_cuda_context(): # Additionally added
#     for gpu_id in range(torch.cuda.device_count()):
#         torch.cuda.set_device(gpu_id)
#         torch.tensor([0.0], device=f'cuda:{gpu_id}') # or another simple CUDA operation ...
#     return


def get_gpu_info(print_info=False): # Additionally added
...
class Memory_Buffer(object): # Additionally added
...
#         self.old_allocated = 0.0
#         self.old_reserved = 0.0

#     def __call__(self, operation: str):

#         device, device_name = torch.cuda.current_device(), torch.cuda.get_device_name(torch.cuda.current_device())
#         assert self.device == device, "Error: A rank mismatch occurred during memory logging!"
#         assert self.device_name == device_name, "Error: A name mismatch occurred during memory logging!"

#         mem_allocated = bytes_to_mib(torch.cuda.memory_allocated(device))
#         mem_reserved = bytes_to_mib(torch.cuda.memory_reserved(device))
#         print(f"\nGPU Memory Info after Operation: [{operation}] @ Device: {device_name} (Rank {device})")
#         print(f"TOTAL Memory Usage: {mem_allocated} MiB allocated and {mem_reserved} MiB reserved")

#         diff_allocated = np.round(mem_allocated - self.old_allocated, 3)
#         diff_reserved = np.round(mem_reserved - self.old_reserved, 3)
#         print(f"GPU Memory Difference after Operation: [{operation}] @ Device: {device_name} (Rank {device})")
#         print(f"DIFFERENCE in Memory Usage: {diff_allocated} MiB allocated and {diff_reserved} MiB reserved")

#         self.old_allocated, self.old_reserved = mem_allocated, mem_reserved # Update step

#         return


def bool_flag(s): # Parse boolean arguments from the command line.
...
def get_sha(): # Secure Hash Algorithm (SHA)
    """
...
def preprocess_image(_img, clahe=False, clahe_clip_limit=3.0, clahe_window=(8,8), subtract_lowpass=True, dtype='float32'):
    ...
    # assert (_img.ndim == 2) and (_img.dtype == np.uint16) # Deactivated since dtype is np.float32 already
    assert (_img.ndim == 2)
